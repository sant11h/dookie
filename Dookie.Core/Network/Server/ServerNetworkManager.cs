...
public class ServerNetworkManager : INetEventListener
{
    ...
    public void Start()
    {
        if (_netManager.IsRunning)
            return;
        _netManager.Start(NetworkConstants.ServerPort);
        _logicTimer.Start();
    }
    ...
    {
        _serverTick = (ushort)((_serverTick + 1) % NetworkConstants.MaxGameSequence);
        ...
        if (_serverTick % 2 == 0)
        {
            ...
            {
                int statesMax = p.AssociatedPeer.GetMaxSinglePacketSize(DeliveryMethod.Unreliable) -
                ...
                for (int s = 0; s < (pCount - 1) / statesMax + 1; s++)
                {
                    ...
                    p.AssociatedPeer.Send(WriteSerializable(PacketType.ServerState, _serverState),
    ...
    {
        Console.WriteLine("[S] Player connected: " + peer);
    }
        ...
        DeliveryMethod deliveryMethod)
    {
        ...
        switch (pt)
        {
            ...
            case PacketType.Serialized:
                ...
                break;
            default:
                Console.WriteLine("[S] Unhandled packet: " + pt);
                break;
        }
    }
    ...
    {
        request.AcceptIfKey(NetworkConstants.ServerKey);
    }
}